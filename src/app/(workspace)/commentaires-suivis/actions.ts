...
function refreshPage() {
  revalidatePath("/commentaires-suivis");
}

export async function createCommentaryEntry(formData: FormData) {
  const me = await getSessionPrismaUser();
  if (!me) redirect("/login?next=/commentaires-suivis");

  const subject = String(formData.get("subject") ?? "").trim();
  if (!subject) redirect("/commentaires-suivis?error=" + encodeURIComponent("L'objet est obligatoire."));
  ...
  refreshPage();
  redirect("/commentaires-suivis?commentCreated=1");
}
...
export async function updateCommentaryEntry(formData: FormData) {
  ...
  const subject = String(formData.get("subject") ?? "").trim();
  if (!id || !subject) {
    redirect("/commentaires-suivis?error=" + encodeURIComponent("Données commentaire invalides."));
  }
  ...
  refreshPage();
  redirect("/commentaires-suivis?commentUpdated=1");
}
...
export async function createFollowUpEntry(formData: FormData) {
  ...
  refreshPage();
  redirect("/commentaires-suivis?followCreated=1");
}
...
export async function updateFollowUpEntry(formData: FormData) {
  ...
  const subject = String(formData.get("subject") ?? "").trim();
  if (!id || !subject) {
    redirect("/commentaires-suivis?error=" + encodeURIComponent("Données suivi invalides."));
  }
  ...
  refreshPage();
  redirect("/commentaires-suivis?followUpdated=1");
}
