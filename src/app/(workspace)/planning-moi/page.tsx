...
export default async function MyPlanningPage({ searchParams }: Props) {
  ...
  const myTemplateNumbers = Array.from(
    new Set(
      [me.planningTemplateNumber, me.planningTemplateNumberA, me.planningTemplateNumberB].filter(
  ...
  const [assignments, shifts, myTemplates, planningWeeks, comments] = await Promise.all([
    ...
    myTemplateNumbers.length
      ? prisma.planningTemplate.findMany({
          where: { number: { in: myTemplateNumbers } },
  ...
  const legendShiftIds = new Set<string>(assignments.map((a) => a.shiftType.id));
  for (const sid of templateShiftByNumberAndOffset.values()) legendShiftIds.add(sid);
  const legendShifts = Array.from(legendShiftIds)
    .map((sid) => shiftById.get(sid))
    ...
    <main className="mx-auto w-full max-w-3xl flex-1 p-4 md:p-8">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/planning-moi?year=${prev.y}&month=${prev.m}`}
          ...
          <Link
            href={`/planning-moi?year=${next.y}&month=${next.m}`}
            className="rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50"
          >
            &rarr;
          </Link>
          <form action="/planning-moi" method="get" className="ml-1 flex items-center gap-1.5">
            <select
              name="month"
      ...
      <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-7">
          ...
          {Array.from({ length: lastDay }, (_, i) => {
            ...
            const offset = getTemplateDayOffsetFromDate(date);
            const effectiveTemplateNumber = getEffectivePlanningConfigForDate(me, date).templateNumber;
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {legendShifts.map((s) => (
          <span
            key={s.id}
            className="inline-flex items-center gap-1 rounded border border-zinc-200 px-2 py-0.5 text-[11px] font-semibold text-zinc-800"
